...
async def verify_session(request: Request):
    """
    Vérifie si le cookie de session créateur est encore valide.
    Le frontend appelle cet endpoint au chargement pour savoir si la session est active.
    Retourne : { valid: bool, expires_at: str | null }
    """
    from .services.session_service import get_session_from_request, is_session_valid, session_expires_at
    ...
    if not token:
        return {"valid": False, "expires_at": None, "reason": "no_cookie"}

    # Vérifier que le token correspond à un tournoi existant
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            select(Tournament).where(Tournament.creator_session == token)
        )
        tournament = result.scalar_one_or_none()

    if not tournament:
        return {"valid": False, "expires_at": None, "reason": "not_found"}

    created = tournament.created_at
    if not is_session_valid(created):
        return {"valid": False, "expires_at": None, "reason": "expired"}
